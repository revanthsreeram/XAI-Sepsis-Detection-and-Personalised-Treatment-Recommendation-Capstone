...
from Recommender.src.util import llprint, multi_label_metric, ddi_rate_score, get_n_params, parameter_report, Regularization, \
    graph_report
...
def Evaluate(model, device, data_eval):

    model = model.to(device).eval()
    smm_record = list()

    for step, input_seq in enumerate(data_eval):
        y_pred, y_pred_prob, y_pred_label = [], [], []
